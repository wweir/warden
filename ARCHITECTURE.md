...
- 托管安装只在配置不存在时生成可启动的 bootstrap 配置；安装时会显式询问是否对外提供服务。默认只监听 `127.0.0.1:9832`，后台入口为 `http://localhost:9832/_admin/`，并启用用户名 `admin`、密码 `admin` 的本机管理后台。若用户选择对外提供服务，则监听 `:9832`，但 bootstrap 配置不写入 `admin_password`，管理后台保持禁用直到用户手动设置强密码。bootstrap 配置不包含 provider / route，避免首次启动隐式依赖外部服务或本地 OAuth 凭证
