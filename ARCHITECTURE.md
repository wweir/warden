...
- `cliproxy.enabled` 启用时，Warden 会在主网关监听前启动一个嵌入式 CLIProxyAPI/cliproxy 服务；其监听地址来自 `backend: cliproxy` provider 的 `http://loopback:port/v1` URL，所有 cliproxy backend provider 必须共享同一个 endpoint
...
- provider family 只承担“上游适配器”职责：`openai => chat + responses_* + embeddings`，开启 `anthropic_to_chat` 时额外支持 `anthropic`；`anthropic => chat + anthropic`；`qwen/copilot => chat`
