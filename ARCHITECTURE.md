...
- 托管安装只在配置不存在时生成可启动的 bootstrap 配置；安装时会显式询问是否对外提供服务。默认只监听 `127.0.0.1:9832`，后台入口为 `http://localhost:9832/_admin/`，并启用用户名 `admin`、密码 `admin` 的本机管理后台。若用户选择对外提供服务，则监听 `:9832`，但 bootstrap 配置不写入 `admin_password`，管理后台保持禁用直到用户手动设置强密码。`make install` 走 `warden -i -y`，跳过交互确认并启动或重启托管服务，但不隐式开启外部监听。bootstrap 配置不包含 provider / route，避免首次启动隐式依赖外部服务或本地 OAuth 凭证
...
- `copilot` 若未显式设置 `api_key`，则从本地 `config_dir` 读取 OAuth 凭证；配置阶段的可读性校验带显式短超时，避免未来慢 I/O 把 startup/reload 卡成无界阻塞
...
- `internal/providerauth` 子包承载 provider 出站鉴权头和静态 header 注入，避免 `selector` 同时承担 provider 选择和 HTTP credential 处理
...
- provider 配置页把接入类型、连接、认证和“可用接口”作为常用配置主路径；`family` / `backend` / `backend_provider` 只在选择“自定义接入”时展开，`service_protocols` 与兼容开关只在选择“自定义接口”时展开；静态模型基线和高级字段直接展示在保存配置的主表单中，高级字段不再重复暴露接入身份或能力配置，只保留网络和 HTTP 头等低频字段。cliproxy 预设隐藏底层 endpoint / adapter 字段，并默认写入 chat-only 能力，避免把 CLIProxyAPI 的原生 Claude/Gemini/Responses 路由误认为 Warden 已经全部一等接入。
