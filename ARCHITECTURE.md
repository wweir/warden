...
其中 `Logs` 页面采用“左侧 route 过滤器 + 右侧日志表 + 详情弹层”的排障结构：左侧简化为纯 route 分组列表，点击 route 即过滤右侧日志；右侧在日志表前增加 scope 摘要条，显式呈现当前 route / 时间范围 / 请求数；由于后端已按 session 去重，每条记录自然平铺，不再需要 session 展开/折叠交互；详情弹层拆成摘要、会话过程和响应结果三段，避免把实时列表和请求排查混在同一平面里。
