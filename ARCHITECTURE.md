...
- `Chat` 页面按 `route.protocol` 选择请求协议：`chat -> /chat/completions`、`responses_* -> /responses`、`anthropic -> /messages`；管理端发起的 Responses 请求必须显式携带 `store=false`，避免本地 Codex/CLIProxy 后端拒绝缺省存储语义
...
- `responses_stateless` 暴露无状态 `/responses`；当 route 至少包含一个支持 embeddings 的 upstream/provider 时，额外暴露 `/embeddings`
- `responses_stateful` 暴露原生 `/responses`，同时允许有状态和无状态请求；当 route 至少包含一个支持 embeddings 的 upstream/provider 时，额外暴露 `/embeddings`
...
- provider family 只承担“上游适配器”职责：`openai => chat + responses_* + embeddings`，开启 `anthropic_to_chat` 时额外支持 `anthropic`；`anthropic => chat + anthropic`；`copilot => chat`
