...
- `cliproxy.auth_dir` 留空默认使用 `/etc/warden`，与 Linux 托管主配置 `/etc/warden/warden.toml` 共享目录；CLIProxyAPI 运行时配置仍由 Warden 单独生成，不直接复用 Warden 主配置文件。该临时运行时文件因 CLIProxyAPI SDK watcher 自身 schema 仍为 YAML，但它不是 Warden 用户配置格式。provider 详情页里的 cliproxy 认证导入只会把完整 CLIProxyAPI auth JSON 写入 `cliproxy.auth_dir`，不会把认证内容落进 provider 配置。导入和列表展示只做离线结构校验，例如 JSON 对象、`type`、常见凭证字段、禁用和本地过期时间提示；不会默认向上游发起 token 刷新、账号探测或模型请求。管理端提供手动在线验证按钮，触发后由后端通过当前 cliproxy provider 的正常 OpenAI-compatible Responses 探测链路发出 `model`、`input` 和 `store=false` 最小请求；由于 CLIProxyAPI HTTP 表面不暴露按 auth 文件强制选中凭证的普通请求参数，该验证证明当前 provider 认证池在线可用，不承诺某个单独文件必然被选中
