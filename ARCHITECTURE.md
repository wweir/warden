...
- `cliproxy.enabled` 启用时，Warden 会在主网关监听前启动一个嵌入式 CLIProxyAPI/cliproxy 服务；其监听地址来自 `backend: cliproxy` provider 的 `http://loopback:port/v1` URL，所有 cliproxy backend provider 必须共享同一个 endpoint；该 URL 是 Warden 到内嵌/本地 cliproxy 的内部 HTTP 边界，不代表还需要额外部署外部模型服务
- `cliproxy.auth_dir` 留空默认使用 `/etc/warden`，与 Linux 托管主配置 `/etc/warden/warden.toml` 共享目录；CLIProxyAPI 运行时配置仍由 Warden 单独生成，不直接复用 Warden 主配置文件。该临时运行时文件因 CLIProxyAPI SDK watcher 自身 schema 仍为 YAML，但它不是 Warden 用户配置格式
