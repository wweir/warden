...
- 支持 `chat`、`responses`、`anthropic` 三种 route 协议
- 支持 OpenAI-compatible `/embeddings`；`chat` / `responses` / `anthropic` route 只有在至少存在一个 embeddings-capable upstream/provider 时才会暴露该入口
...
- `responses`：`<prefix>/responses`（无状态请求走 inference 路径，带 `previous_response_id` 的有状态请求走透明转发）
- `anthropic`：`<prefix>/messages`
- `chat` / `responses` / `anthropic`：`<prefix>/embeddings`
- 任意 route 的公开模型列表：`<prefix>/models`
  - exact model 会直接列出；wildcard model 会合并 provider `/models` 发现结果，以及运行时已经命中过的具体模型
  - 返回体同时包含 OpenAI-compatible `data` 字段和 Codex CLI catalog 兼容的 `models` 字段
...
如果你另外定义了 `responses` route，再调用 `<route-prefix>/responses`：
...
- `route.protocol` 必须显式声明，而且只能是 `chat`、`responses`、`anthropic` 之一
...
- `responses` 协议的 route 接受无状态请求(走 inference handler,支持 failover、tool hook、`responses_to_chat` 桥接)和带 `previous_response_id` 的有状态请求(走透明转发,禁用 failover,不解析 tool calls)
...
- Supports three route protocols: `chat`, `responses`, and `anthropic`
- Supports OpenAI-compatible `/embeddings`; `chat` / `responses` / `anthropic` routes expose it only when the route contains at least one embeddings-capable upstream/provider
...
- `chat`: `<prefix>/chat/completions`
- `responses`: `<prefix>/responses` (stateless requests go through the inference path; requests carrying `previous_response_id` go through transparent forwarding)
- `anthropic`: `<prefix>/messages`
- Public model list for any route: `<prefix>/models`
  ...
  - The response includes both the OpenAI-compatible `data` field and the Codex CLI catalog-compatible `models` field.
...
If you define a `responses` route, call `<route-prefix>/responses`:
...
- `route.protocol` is required and must be one of `chat`, `responses`, or `anthropic`
...
- `responses` routes accept stateless requests (handled by the inference pipeline) and stateful requests carrying `previous_response_id` (forwarded transparently with failover disabled, no tool-call parsing)
