...
- Plain upstream `400` validation errors are returned to the client instead of being retried; same-provider fallback retries are capped to one attempt per request when no alternative provider is available.
