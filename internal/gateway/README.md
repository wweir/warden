...
2. `dashboardMetricsStore` samples cumulative counters and output-rate gauges every 2 seconds.
3. The store converts counter deltas into usage/error points, stores total output TPS plus per-provider output TPS samples, and also keeps per-route request-rate, error-rate, and output-rate time series for the routes page.
