...
- `responses` routes expose `/responses` for both stateless and stateful requests. Stateless requests (no `previous_response_id`) go through the inference handler with failover, tool hooks, and `responses_to_chat` bridging where applicable. Stateful requests (carrying `previous_response_id`) are dispatched to the transparent forwarding path, which preserves model rewrites, header injection, and request logging but disables failover and skips tool-hook parsing because upstream owns the conversation state.
- `responses` routes also expose `/embeddings` only when the route has at least one embeddings-capable upstream/provider.
...
- `embeddings` keeps OpenAI-compatible JSON on every route; native `anthropic` providers are filtered out for that service protocol, so `route.protocol=anthropic` only serves embeddings when the matched model resolves to an OpenAI-family provider. The handler validates only `model` and `input`, rewrites the model when needed, and forwards standard or provider-specific fields such as `encoding_format`, `dimensions`, `user`, and `input_type` without rebuilding the request shape.
...
- Provider family compatibility is derived centrally from provider config: `openai => chat + responses + embeddings` plus optional `anthropic` when `anthropic_to_chat` is enabled, `anthropic => chat + anthropic`, `copilot => chat`.
...
- Plain upstream `400` validation errors are returned to the client instead of being retried; upstream `402 Payment Required` is treated as a provider payment/quota failure and can fail over; same-provider fallback retries are capped to one attempt per request when no alternative provider is available.
