		...
		Presets: []providerFormPresetMeta{
			{
				...
				Summary:                 "OpenAI official /v1 endpoint with full OpenAI-compatible defaults.",
			...
			},
			{
				...
				Summary:                 "Anthropic native /v1 endpoint with chat + anthropic defaults.",
			...
			},
			{
				...
				Summary:                 "Generic OpenAI-compatible upstream. Good for custom vendors and self-hosted gateways.",
				Family:                  config.ProviderProtocolOpenAI,
				AuthMode:                "api_key",
				ServiceProtocolTemplate: "adapter_defaults",
			},
			...
			},
			{
				...
				Summary:                 "OpenAI-compatible cliproxy sidecar backed by Codex.",
				Family:                  config.ProviderProtocolOpenAI,
			...
			},
			{
				...
				Summary:                 "OpenAI-compatible cliproxy sidecar backed by Claude CLI credentials.",
				Family:                  config.ProviderProtocolOpenAI,
				Backend:                 config.ProviderBackendCLIProxy,
			...
			},
			{
				...
				Summary:                 "OpenAI-compatible cliproxy sidecar backed by Gemini CLI credentials.",
				Family:                  config.ProviderProtocolOpenAI,
				Backend:                 config.ProviderBackendCLIProxy,
			...
			},
			{
				ID:                      "qwen-cli",
				Title:                   "Qwen CLI",
				Summary:                 "Qwen adapter using local CLI credentials from config_dir.",
				Family:                  config.ProviderProtocolQwen,
				AuthMode:                "config_dir",
				DefaultURL:              "https://portal.qwen.ai/v1",
				DefaultConfigDir:        "~/.qwen",
				ServiceProtocolTemplate: "adapter_defaults",
			},
			{
				ID:                      "copilot-cli",
		...
		ServiceProtocolTemplates: []providerFormServiceProtocolTemplateMeta{
			{
				...
				Families:         []string{config.ProviderProtocolOpenAI, config.ProviderProtocolAnthropic, config.ProviderProtocolQwen, config.ProviderProtocolCopilot},
				Backends:         []string{""},
			...
			},
			{
				ID:               "chat_only",
				Title:            "Chat Only",
				Summary:          "Expose chat only. Recommended for Ollama and cliproxy-backed providers unless other surfaces are verified.",
				Families:         []string{config.ProviderProtocolOpenAI, config.ProviderProtocolAnthropic, config.ProviderProtocolQwen, config.ProviderProtocolCopilot},
