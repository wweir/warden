...
func TestFromJSONOpenAIUsage(t *testing.T) {
	...
	obs := FromJSON([]byte(`{"usage":{"prompt_tokens":3,"completion_tokens":5,"total_tokens":8}}`))
	if obs.CompletenessLabel() != CompletenessExact {
		t.Fatalf("completeness = %q, want %q", obs.CompletenessLabel(), CompletenessExact)
	}
	if obs.PromptTokens != 3 || obs.CompletionTokens != 5 || obs.TotalTokens != 8 {
...
func TestFromJSONResponsesUsage(t *testing.T) {
	...
	obs := FromJSON([]byte(`{"usage":{"input_tokens":7,"output_tokens":11,"total_tokens":18}}`))
	if obs.CompletenessLabel() != CompletenessExact {
		t.Fatalf("completeness = %q, want %q", obs.CompletenessLabel(), CompletenessExact)
	}
	if obs.PromptTokens != 7 || obs.CompletionTokens != 11 || obs.TotalTokens != 18 {
		t.Fatalf("unexpected observation: %+v", obs)
	}
}
...
func TestFromOpenAIChatStream(t *testing.T) {
	...
	raw := []byte("data: {\"id\":\"chatcmpl_1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"ok\"},\"finish_reason\":null}]}\n\n" +
		"data: {\"id\":\"chatcmpl_1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":3,\"completion_tokens\":5,\"total_tokens\":8}}\n\n" +
		"data: [DONE]\n\n")
	...
	if obs.PromptTokens != 3 || obs.CompletionTokens != 5 {
		t.Fatalf("unexpected observation: %+v", obs)
	}
}
...
func TestFromResponsesStream(t *testing.T) {
	...
	raw := []byte("data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_1\",\"status\":\"in_progress\"}}\n\n" +
		"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_1\",\"status\":\"completed\",\"usage\":{\"input_tokens\":2,\"output_tokens\":4,\"total_tokens\":6}}}\n\n")

	obs := FromResponsesStream(raw)
	if obs.CompletenessLabel() != CompletenessExact {
		t.Fatalf("completeness = %q, want %q", obs.CompletenessLabel(), CompletenessExact)
	}
	if obs.PromptTokens != 2 || obs.CompletionTokens != 4 {
		t.Fatalf("unexpected observation: %+v", obs)
	}
}
...
func TestFromAnthropicStream(t *testing.T) {
	...
	if obs.PromptTokens != 10 || obs.CompletionTokens != 5 {
		t.Fatalf("unexpected observation: %+v", obs)
	}
}
