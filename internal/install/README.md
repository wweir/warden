...
- Creates the minimal managed bootstrap config when no config exists. Interactive installs prompt for and confirm the admin password before writing the config. On Linux the managed config path is `/etc/warden/warden.toml`; macOS and Windows use platform-appropriate TOML paths under `/usr/local/etc/warden` and `C:\ProgramData\Warden`.
