...
- provider 编辑器把 `family` 作为推荐字段，并允许通过 `enabled_protocols` / `disabled_protocols` 静态收窄该 provider 可参与的 route 协议面；对 `openai` provider 还会暴露 `responses_to_chat` 与 `anthropic_to_chat` 桥接开关
