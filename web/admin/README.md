...
- provider 编辑器把 `family` 作为推荐字段，并暴露当前真实支持的 provider 配置项：`url`、`api_key`、`config_dir`、`proxy`、`headers`、`models`；对 `openai` provider 额外暴露 `responses_to_chat` 与 `anthropic_to_chat` 桥接开关
